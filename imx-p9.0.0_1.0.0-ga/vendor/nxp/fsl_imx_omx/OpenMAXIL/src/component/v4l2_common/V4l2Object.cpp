...
OMX_ERRORTYPE V4l2Object::GetFormat(V4l2ObjectFormat * pFormat)
{
    ...
    if(pFormat->width > FRMAE_WIDTH_LIMITATION || pFormat->height > FRMAE_HEIGHT_LIMITATION || pFormat->stride > FRMAE_WIDTH_LIMITATION){
...
OMX_ERRORTYPE V4l2Object::SetBuffer(OMX_BUFFERHEADERTYPE * inputBuffer,OMX_U32 flags)
{
    ...
    if(inputBuffer == NULL || inputBuffer->pBuffer == NULL/* || inputBuffer->nAllocLen < 
        nBufferSize*/){
        LOG_ERROR("[%p]SetBuffer error,%p,len=%d,nBufferSize[0]=%d \n",this,
            inputBuffer->pBuffer,inputBuffer->nAllocLen,nBufferSize[0]);
        return OMX_ErrorBadParameter;
    }else
...
OMX_U32 V4l2Object::ProcessBuffer(OMX_U32 flags)
{
    ...
    if(!bRunning){
        ...
        fsl_osal_mutex_unlock(sMutex);
        LOG_ERROR("[%p]do not call dqbuf after stream off\n",this);
    ...
    if(pBufferMap[outIndex].omxBuffer != NULL){
        ...
        ts = stV4lBuf.timestamp.tv_sec *1000000;
...
OMX_ERRORTYPE V4l2Object::SetBuffer(DmaBufferHdr * inputBuffer,OMX_U32 flags)
{
    ...
    if(inputBuffer == NULL || inputBuffer->num_of_plane != 2){
