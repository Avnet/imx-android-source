...
void *Process3ThreadHandler(void *arg)
{
    ...
    volatile PostProcess3Cmd * pCmd = NULL;
    ...
    pCmd=&base->ePostCmd;
    if(*pCmd!=POST_PROCESS3_CMD_NONE){
        switch(*pCmd){
            case POST_PROCESS3_CMD_RUN:
                base->bRunning = OMX_TRUE;
                //fall through here
            case POST_PROCESS3_CMD_STOP:
            case POST_PROCESS3_CMD_FLUSH:
            {
                *pCmd=POST_PROCESS3_CMD_NONE;
                fsl_osal_sem_post(base->pPostCmdSem);
                return NULL;
            }
...
OMX_ERRORTYPE Process3::Create(PROCESS3_BUFFER_MODE buf_mode)
{
    ...
    ePostCmd = POST_PROCESS3_CMD_NONE;
...
OMX_ERRORTYPE Process3::Start()
{
    LOG_DEBUG("Process3::Start \n");

    ePostCmd = POST_PROCESS3_CMD_RUN;
    pThread->run_once();
    if(!bRunning)
        return OMX_ErrorNone;

    fsl_osal_sem_wait(pPostCmdSem);
...
OMX_ERRORTYPE Process3::Flush()
{
    ...
    ePostCmd = POST_PROCESS3_CMD_FLUSH;
    pThread->run_once();

    //wait for flush command handled, sometimes the wait is triggered by start signal.
    while(POST_PROCESS3_CMD_FLUSH == ePostCmd){
        fsl_osal_sem_wait(pPostCmdSem);
    }

    LOG_DEBUG("Process3::Flush end ePostCmd=%d\n",ePostCmd);
