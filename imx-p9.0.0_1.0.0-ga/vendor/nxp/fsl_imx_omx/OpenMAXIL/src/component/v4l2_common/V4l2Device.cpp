...
OMX_S32 V4l2Dev::Open(OMX_U8* name)
{
    OMX_S32 fd = -1;
    int ret = 0;
    fd = open ((char*)name, O_RDWR);
    if(fd > 0){
        ...
        if(ret < 0){
            LOG_DEBUG("V4l2Dev::Open VIDIOC_SUBSCRIBE_EVENT failed.\n");
        }
    }
    return fd;
}
...
OMX_ERRORTYPE V4l2Dev::SetEncoderPrama(OMX_S32 fd,V4l2EncInputParam *param)
{
    ...
    LOG_DEBUG("SetEncoderPrama nBitRate=%d\n",param->nBitRate);
    LOG_DEBUG("SetEncoderPrama nGOPSize=%d\n",param->nGOPSize);
    LOG_DEBUG("SetEncoderPrama nIntraFreshNum=%d\n",param->nIntraFreshNum);
    if(param->nBitRate > 0)
